...
#            total_loss.append(loss)
    def cal_results(idx, file):
        print("MSSL Idx:", idx)
        file1 = os.path.join(TEMP_CACHE,file)
        file2 = os.path.join(results_dir,file.rsplit('.', 1)[0] + '.wav')
        return compute_mssl(file1,file2,[2048, 1024, 512, 256, 128, 64])

    with cf.ThreadPoolExecutor(max_workers=50) as exe:
        total_loss = list(exe.map(lambda x: cal_results(x[0], x[1]), enumerate(os.listdir(source_dir))))
