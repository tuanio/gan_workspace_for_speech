...
def normalize(sig1, sig2):
    ...
    def get_mediainfo(sig):
        data, rate = librosa.load(sig, sr=None)
    ...
    sound1 = AudioSegment.from_file(sig1)
    sound2 = AudioSegment.from_file(sig2)
...
def norm_and_LSD(file1, file2):
    ...
    aud_1, _ = librosa.load(file1, sr=None)
    aud_2, _ = librosa.load(file2, sr=None)
...
def main(source_dir=defaults["test_source"],results_dir=defaults["test_results"], use_gender = defaults["use_gender_test"],csv_path=defaults["annotations"]):
    ...
    file2 = os.path.join(results_dir,file_0.rsplit('.', 1)[0] + '.wav')
...
#        file2 = os.path.join(results_dir,file.rsplit('.', 1)[0] + '.flac')
...
#        total_loss.append(lsd)
    def cal_lsd(idx, file):
        print("Idx:", idx)
        file1 = os.path.join(TEMP_CACHE,file)
        file2 = os.path.join(results_dir,file.rsplit('.', 1)[0] + '.wav')
        return norm_and_LSD(file1, file2).item()

    with cf.ThreadPoolExecutor(max_workers=50) as exe:
        total_loss = list(exe.map(lambda x: cal_lsd(x[0], x[1]), enumerate(os.listdir(source_dir))))
    
    return total_loss

    # total = sum(total_loss)
    # min_file = sorted(list_results, key=lambda x: x[0])[0][1]
    ...
    if use_gender:
        ...
        return total_mean, total_std, total_loss
