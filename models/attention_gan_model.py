...
class AttentionGANModel(BaseModel):
    ...
    def forward(self):
        ...
        if not self.opt.use_mask:
            ...
            self.fake_B, *_ = self.netG_A(self.real_A, self.A_mask)  # G_A(A)
            self.rec_A, *_ = self.netG_B(self.fake_B, torch.ones(self.fake_B.size(0),1,self.fake_B.size(2),self.fake_B.size(3)))   # G_B(G_A(A))

            self.fake_A, *_ = self.netG_B(self.real_B, self.B_mask)  # G_B(B)
            self.rec_B, *_ = self.netG_A(self.fake_A , torch.ones(self.fake_A.size(0),1,self.fake_A.size(2),self.fake_A.size(3)))   # G_A(G_B(B))
    ...
    def backward_G(self):
        ...
        if lambda_idt > 0:
            ...
            if self.opt.use_mask:
                self.idt_A, *_  = self.netG_A(self.real_B, torch.ones(self.real_B.size(0),1,self.real_B.size(2),self.real_B.size(3)))
                self.loss_idt_A = self.criterionIdt(self.idt_A, self.real_B) * lambda_B * lambda_idt
                # G_B should be identity if real_A is fed: ||G_B(A) - A||
                self.idt_B, *_  = self.netG_B(self.real_A, torch.ones(self.real_A.size(0),1,self.real_A.size(2),self.real_A.size(3)))
